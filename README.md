...
- **Autonomía (Self-Service):** [5/5] - _No fue necesaria ayuda de terceros_
- **Claridad de Documentación:** [5/5] - _El README es preciso y está actualizado_
- **Eficiencia del Toolchain:** [3/5] - _Tomó demasiado tiempo la instalación de dependencias_
- **Postura de Seguridad:** [3/5] - _Varias vulnerabilidades altas y críticas_
- **Consistencia de Dependencias:** [5/5] - _No se presentaron conflictos entre versiones_
- **Feedback del Sistema:** [5/5] - _Los logs indican que la aplicacióin está corriendo de forma clara_
...
## 1. Baseline de despliegue (estado inicial)

Antes de las mejoras de DevEx de esta entrega, el proyecto base OWASP Juice Shop ya incluía opciones de despliegue con contenedores:

- En `juice-shop/README.md` existe la sección **Docker Container** con ejecución directa vía `docker pull` y `docker run`.
- En `juice-shop/Dockerfile` se define la imagen de la aplicación (build por etapas y runtime distroless).
- En `juice-shop/docker-compose.test.yml` ya se utilizaba Compose para pruebas de humo (`sut` + `app`).

Esto confirma que Docker no se introdujo desde cero en nuestro proyecto: lo que se hizo en Delivery 4 fue **estandarizar y simplificar** su uso para el equipo con un flujo de "one-command setup".

## 2. One-Command Setup (estandarización DevEx)
...
## 3. Strategic ADR (alineado a ADR-0002)

### Decisión arquitectónica propuesta

Tal como se documenta en `ADR-002-postgresql-migration.md` (estado `Proposed`), se propone:

> **Migrar la persistencia principal a PostgreSQL 16 para Integración y Producción, manteniendo SQLite exclusivamente para desarrollo local y laboratorios.**

### Contexto y evidencia del ADR

- El problema identificado es arquitectónico: App + SQLite comparten proceso, generando contención y fragilidad (`SQLITE_BUSY`, sincronización destructiva del esquema).
- Los benchmarks reportan degradación severa en carga alta y fallas por OOM bajo estrés.

| Escenario        | Carga / Conexiones | Throughput  | Latencia (p99) | Errores / Timeouts |
| ---------------- | ------------------ | ----------- | -------------- | ------------------ |
| Baseline (Baja)  | 10 con / 5s        | 214.3 req/s | 83.5 ms        | 0 / 0              |
| Estrés (Search)  | 100 con / 20s      | 14.53 req/s | 760.4 ms       | 6,361 / 600        |
| Estrés (Reviews) | 100 con / 20s      | 99.46 req/s | 1,479.2 ms     | 36,491 / 114       |

### Principios de diseño definidos

1. Desacoplar proceso de cómputo (Node.js) y motor de datos.
2. Habilitar escalabilidad horizontal en la capa de aplicación.
3. Implementar gobernanza de esquema con migraciones versionadas.
4. Ejecutar transición gradual con criterios de salida medibles.

### Trade-offs principales

| Característica         | SQLite (Status Quo)                 | PostgreSQL 16 (Propuesta)             |
| ---------------------- | ----------------------------------- | ------------------------------------- |
| Modelo de concurrencia | Bloqueos por contención de archivo  | Transacciones concurrentes (MVCC)     |
| Gestión de esquema     | Sincronización destructiva (manual) | Migraciones versionadas (automáticas) |
| Escalabilidad          | Vertical (limitada por el proceso)  | Horizontal (app nodes independientes) |
| Operación              | Simple, pero opaca y frágil         | Compleja, pero gobernable y robusta   |
| Costo inicial          | Bajo                                | Medio                                 |

### Riesgos y mitigación

- **RSK-001 - Incompatibilidades de dialecto:** pruebas cruzadas obligatorias en CI.
- **RSK-002 - Errores de migración de datos:** ciclo staging, dry run y backup previo.
- **RSK-003 - Regresiones de performance:** benchmarks continuos por Pull Request.
- **RSK-004 - Complejidad operativa:** runbooks de observabilidad y ownership explícito.

### Plan de implementación por fases

- **Fase 1 (Semana 1):** parametrizar conexiones y mantener SQLite local. Salida: smoke tests en ambos dialectos.
- **Fase 2 (Semana 2):** introducir `sequelize-cli` y job CI con PostgreSQL. Salida: migraciones idempotentes y rollback validado.
- **Fase 3 (Semana 3):** hardening (pool, índices, benchmarks). Salida: mejora sostenida en p95/p99 y menos timeouts.
- **Fase 4 (Semana 4):** cutover progresivo y monitoreo intensivo. Salida: SLO de error/latencia estabilizado.

### Esfuerzo y costos estimados

- **Esfuerzo técnico:** 46 - 66 horas hombre.
- **Horizonte temporal:** 3 - 4 semanas.
- **Costo operativo recurrente:** servicio PostgreSQL + monitoreo + backups.

Perfil de costo mensual estimado para producción inicial (2 vCPU / 8 GB RAM):

| Propuesta                   | Costo mensual est. |
| --------------------------- | ------------------ |
| DigitalOcean (Plan DB-XL)   | $120.00            |
| Supabase Pro (Large)        | $136.50            |
| AWS RDS (t4g.large + Proxy) | $158.23            |

Para el detalle técnico y justificación completa, ver `ADR-002-postgresql-migration.md`.
...
- **Herramienta de Benchmarking:** Se utilizó **Autocannon** (vía `npx autocannon`) para medir la capacidad real del _Event Loop_.
...
- **Muestreo Estocástico:** Para evitar sesgos de "Cold Start" o anomalías temporales de red, se ejecutaron **5 iteraciones consecutivas** para cada escenario (Original vs. Refactorizado), capturando el _Throughput_ y el volumen de transferencia (Bytes/Sec).
...
Al cambiar a la búsqueda directa por atributo (`product: id`), el _Event Loop_ se estabilizó y la base de datos resolvió las peticiones en tiempo logarítmico.
